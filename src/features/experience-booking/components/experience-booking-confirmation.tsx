...
export function ExperienceBookingConfirmation({ booking }: { booking: ExperienceBooking }) {
  return (
    <main className="flex-1">
      <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6 lg:px-8">
        <CheckCircle2 className="mx-auto h-14 w-14 text-primary" />
        <h1 className="mt-4 text-2xl font-bold">Đặt experience thành công</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Mã đặt chỗ #{booking.id} đã được xác nhận.
        </p>

        <div className="mt-8 space-y-3 rounded-[var(--radius-lg)] border border-border bg-card p-6 text-left">
          <div className="flex justify-between">
            <span className="text-sm text-muted-foreground">Experience</span>
            <span className="text-sm font-medium">{booking.experienceTitle}</span>
          </div>
          <div className="flex justify-between">
